class ResponseFollowUpAuditArea {
  List<ModelListFollowUpAuditArea>? followUp;

  ResponseFollowUpAuditArea({this.followUp});

  ResponseFollowUpAuditArea.fromJson(Map<String, dynamic> json) {
    if (json['follow_up'] != null) {
      followUp = <ModelListFollowUpAuditArea>[];
      json['follow_up'].forEach((v) {
        followUp!.add(new ModelListFollowUpAuditArea.fromJson(v));
      });
    }
  }
